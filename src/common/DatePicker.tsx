...
const TimeSegment = ({ segment, state }: TimeSegmentProps) => {
  ...
  return (
    <div
      ...
      className={cx(
        // base
        "relative block w-full appearance-none rounded-lg border px-2.5 py-1.5 text-left uppercase tabular-nums shadow-sm outline-none transition sm:text-sm",
        // border color
...
const triggerStyles = tv({
  base: [
    // base
    "peer flex w-full cursor-pointer appearance-none items-center gap-x-2 truncate rounded-lg border px-3 py-3 shadow-sm outline-none transition-all sm:text-sm",
    // background color
    "bg-white dark:bg-gray-950",
    // border color
    "border-gray-300 dark:border-gray-800",
  ...
  return (
    <PopoverPrimitives.Portal>
      <PopoverPrimitives.Content
        ...
        className={cx(
          // base
          "relative z-50 w-fit rounded-lg border text-sm shadow-xl shadow-black/[2.5%]",
          // widths
        ...
        return (
          <li key={index} className="sm:w-full sm:py-px">
            <button
              ...
              className={cx(
                // base
                "relative w-full overflow-hidden text-ellipsis whitespace-nowrap rounded border px-2.5 py-3 text-left text-base shadow-sm outline-none transition-all sm:border-none sm:py-2 sm:text-sm sm:shadow-none",
                // text color
