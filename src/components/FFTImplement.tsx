...
import CustomizedGradient from '../common/Customisedgradient';
...
const FFTAreaChartStack = () => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'relative',
        width: `${config.CHART_WIDTH}px`,
        height: `${config.CHART_HEIGHT * config.NUM_CHARTS}px`,
      }}>
        {syntheticData.map((series, index) => {
          const maxValue = Math.max(...series.data.map(d => d.amplitude));
          const gradientId = `gradient-${index}`;
          ...
          return (
            <div
              ...
              style={{
                position: 'absolute',
                top: `${index * config.CHART_HEIGHT}px`,
                left: 0,
                width: '100%',
                height: `${config.CHART_HEIGHT}px`,
                zIndex: config.NUM_CHARTS - index,
                opacity: isChartVisible(index) ? 1 : 0.1,
                transition: 'opacity 0.15s ease-in-out',
                pointerEvents: 'auto',
              }}
            >
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis 
                    ...
                    type="number"
                    domain={[0, 1000]}
                    hide={index !== config.NUM_CHARTS - 1}
                    axisLine={{ stroke: '#d3d3d3' }}
                    label={index === config.NUM_CHARTS - 1 ? { value: 'Frequency (Hz)', position: 'bottom' } : null}
                    ticks={config.USE_HARMONIC_X_AXIS ? 
                      config.HARMONICS.map(h => h * config.BASE_FREQUENCY) : 
                      [...Array(Math.floor(1000 / config.X_AXIS_INTERVAL) + 1)].map((_, i) => i * config.X_AXIS_INTERVAL)}
                  />
              ...
              >
                <circle
                  cx={config.CHART_WIDTH - 10}
                  cy={10}
                  r={config.MARKER_SIZE}
                  fill={selectedCharts.includes(index) ? "red" : "#888"}
                />
                <rect 
                  x={config.CHART_WIDTH - 30}
                  y={0}
                  width={30}
                  height={30}
                  fill="transparent"
